...
class PlayerProcessor(ResourceProcessor):
    def __init__(self, logger, contour):
        super(PlayerProcessor, self).__init__(logger, contour)
            ...
            if code == 0:
                if len(result) > 0:
                    if len(result[0].rows) > 0:
                        ...
                        create_r, create_s = self.__create({"id": player_id})
                        if create_s == 201:
                            return {"success": True, "data": schema.dump({"id": player_id})}, 200
                        else:
    ...
    def __create(self, request_body: dict) -> typing.Tuple[dict, int]:
        """Create character, only called from API_GET when player doesn't exist"""

        schema = PlayerSchema(only=("id",))

        try:
            vd = schema.load(request_body)
            ...
            self.logger.error(f"Exception during player CREATE with body {request_body}: {traceback.format_exc()}")
            return self.internal_server_error_response
...
if __name__ == '__main__':
    player_proc = PlayerProcessor(logging.getLogger(__name__), "dev")

    r, s = player_proc.modify("earlydevtestplayerid", 100, 100, 50, 100, "api_test", "")
    print(s, r)
